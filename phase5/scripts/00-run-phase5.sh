...
#
# Everything is bounded by timeouts and every failure is recorded in
# phase5/out/evidence/ — evidence is committed and pushed even on failure.
set -uo pipefail
...
run_c() { timeout "$1" bash -c "${*:2}" 2>&1 | tee -a "$MAINLOG"; local rc=${PIPESTATUS[0]}; [ "$rc" = 0 ] || { echo "STEP_FAILED rc=$rc: ${*:2}" | tee -a "$MAINLOG"; return 1; }; }
PKG="ai.opencode.android.debug"

echo "=== PHASE 5 START $(date -u +%FT%TZ) staged=$STAGED ===" | tee -a "$MAINLOG"
export PATH="${ANDROID_HOME:-/usr/local/lib/android/sdk}/platform-tools:${ANDROID_HOME:-/usr/local/lib/android/sdk}/emulator:${ANDROID_HOME:-/usr/local/lib/android/sdk}/cmdline-tools/latest/bin:$PATH"
...
if [ "$STAGED" = "0" ]; then
  ...
  echo no | "$AVDM" create avd -n phase5 -k "system-images;$API;$TAG;$ABI" --force 2>&1 | tail -3
  # Phase 4's proven boot script, pointed at this phase's AVD name.
  ...
  run_c 1500 "bash '$OUT/boot-emulator.sh'" || { echo "FATAL: emulator did not boot"; exit 1; }

  step "2/6 payload (reuse phase 4's build when it is present)"
  if [ ! -f "$ROOT/phase4/out/engine/assets/runtime-manifest.json" ]; then
    run_c 3600 "bash '$ROOT/phase4/scripts/10-build-payload.sh'" || { echo "FATAL: payload build failed"; exit 1; }
  else
    echo "reusing phase4/out/engine payload (payloadVersion is checked on device)"
  fi
  run_c 900 "bash '$ROOT/phase4/scripts/11-build-mcp.sh'" || echo "warn: phase4 MCP build failed (G10 halves may skip)"
  ...
  run_c 3000 "'$ROOT/gradlew' -p '$ROOT' :app:assembleDebug :app:assembleDebugAndroidTest :app:testDebugUnitTest --no-daemon --stacktrace" \
    || { echo "FATAL: gradle build failed"; exit 1; }
  ...
  [ -f "$APK" ] || { echo "FATAL: no debug APK"; exit 1; }
  adb install -r -g "$APK" 2>&1 | tail -2 | tee -a "$MAINLOG"
  [ -n "$(tapk)" ] && adb install -r -g "$(tapk)" 2>&1 | tail -2 | tee -a "$MAINLOG"
  adb logcat -c 2>/dev/null || true
else
  step "1/6 staged mode: device + payload are phase 4's; build only what Phase 5 needs"
  adb devices | tee -a "$MAINLOG"
  adb shell pm list packages | grep ai.opencode | tee -a "$MAINLOG" || echo "warn: app package not found"

  step "2/6 payload/APK artifacts from the phase 4 stage"
  APK="$(ls "$ROOT/app/build/outputs/apk/debug/"*.apk 2>/dev/null | head -1)"
  echo "app apk: ${APK:-none}"

  step "3/6 androidTest APK + JVM unit tests"
  run_c 3000 "'$ROOT/gradlew' -p '$ROOT' :app:assembleDebug :app:assembleDebugAndroidTest :app:testDebugUnitTest --no-daemon --stacktrace" \
    || { echo "FATAL: gradle build failed"; exit 1; }
  tapk="$(ls "$ROOT/app/build/outputs/apk/androidTest/debug/"*.apk 2>/dev/null | head -1)"
  [ -n "$tapk" ] && adb install -r -g "$tapk" 2>&1 | tail -2 | tee -a "$MAINLOG" || echo "FATAL: no androidTest APK; instrumented gates impossible"
  adb logcat -c 2>/dev/null || true
fi

step "4/6 host-side remote MCP fixture (pinned MCP SDK)"
run_c 900 "bash '$DIR/scripts/11-build-remote-mcp.sh'" || echo "warn: remote MCP fixture build failed (P5-G16 will record the reason)"
...
step "6/6 evidence"
cp "$DIR/out/00-run-phase5.log" "$EV/" 2>/dev/null || true
...
# Commit + push evidence so a green/red run is readable without artifact download.
if [ "${P5_SKIP_PUSH:-0}" != "1" ] && git -C "$ROOT" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  step "evidence push"
