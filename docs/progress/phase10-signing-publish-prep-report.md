...
| `35644154793` | `e879fb0` (the footer-honesty driver — the revision this section ends on) | **SUCCESS**, 32m40s, 19:20:06Z → 19:52:46Z: `phase6_ui_fails=0 phase10_gate_fails=0 phase9_gate_fails=0`, `=== PHASE 10 END 2026-09-21T19:52:19Z rc=0 ===`, `P10_DRIVER_SELFTEST PASS` with the fourteen scenarios, and the runner's own committed `p10-driver-selftest.log` ending `pass=106 fail=0` — i.e. the footer change is verified on the runner, not only here. W1/W2/W3 PASS again on the emulator against `/storage/emulated/0/Documents/OpenCode` (`W2`: `seesOtherProject=false seesOutside=false escapeRefused=true`; `W3`: `projectFile=/storage/emulated/0/Documents/OpenCode/p7-memory-…/AGENTS.md`) |

**Three runs in parallel, by my own carelessness.** The two documentation pushes (`b478eba`
at `35643055959`, SUCCESS 33m; `40d9c32` at `35643166631`, SUCCESS 32m42s) each started a
30-minute suite of their own, and the driver push that followed superseded both while they
were still running — nothing was wrong with them, but the runner time was spent twice for
the same tree. Worth writing down because the same thing happened once before in this
project, and the lesson is cheap: **push documentation with the code, not before it**.
