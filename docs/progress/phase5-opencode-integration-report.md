...
- **BLOCKED (needs the user, not me):** (1) installing the workflow — this session's token cannot create or modify `.github/workflows/` (re-verify below), so `phase5/workflow/phase5-integration.yml` must be copied to `.github/workflows/phase5-integration.yml`; (2) running the suite on the Realme RMX3830 (adb from the user's machine); (3) any repo secret — none is needed for this phase, and `gh secret list` is 403 for this token, so the suite must never depend on one.
