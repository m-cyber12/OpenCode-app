...
|---|---|---|
| Runtime internals app-private | runtime payload, XDG dirs, logs, secrets, credentials under `/data/data/io.github.mcyber12.opencode/files` (mode 0700); no `MANAGE_EXTERNAL_STORAGE` | P5-G18, device `files-layout.txt` (Phase 8 §4 #1) |
| Project files app-specific external (Phase 10 continuation) | projects live under `/storage/emulated/0/Android/data/io.github.mcyber12.opencode/files/workspaces/<project>`: the app's own directory on shared storage, no permission required, deliberately readable by `adb`/MTP so the agent's output is not a black box. Other APPS still cannot read it (platform restriction on Android 11+); the app additionally offers SAF publish to a user-chosen folder, which is the user's explicit decision about their own files | P10_WS_W4_WORKSPACE_VISIBLE + `92-workspace-visibility.sh` (non-root shell lists and reads a file the server wrote); P7 W1-W3 re-run on the new root |
