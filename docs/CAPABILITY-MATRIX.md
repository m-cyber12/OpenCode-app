...
| **File write** (write/edit/patch tools) | native fs | same, inside the project directory (`Documents/OpenCode/<project>` on shared storage since the Phase 10 continuation v3, with All files access; `Android/data/<pkg>/files/workspaces/<project>` when that grant is refused) | TESTED (P5 G-gates, P7 W2 boundary, P10 W4 + non-root shell read/list on the shared root) |
| **Seeing the agent's files** | the filesystem itself | the project folder itself: `Documents/OpenCode/<project>` is readable by the Files app, any file manager, MTP/USB and `adb` while the agent works; in-app file browser (OpenCode's `/file` API) as a convenience view; SAF "Export a copy..." for a snapshot elsewhere | TESTED (P6-U9 file browser + storage-panel gate; P10 W1-W4 + visibility gates including SHARED_ROOT and the Documents-provider query) |
...
| **Project / workspace** (cwd = project, multiple projects, worktrees) | directory chosen at launch | Projects screen; each project = its own directory under the shared-storage root (`Documents/OpenCode`) = its own OpenCode instance (`?directory=`); storage panel (grant All files access / pick a folder / move existing projects); SAF import/export; isolation gate | TESTED (P7 W1-W3, re-run on the new root: P10_WS_*). Worktrees: not exposed |
...
| **Plugins** (`@opencode-ai/plugin`, npm-installed on demand) | installed by upstream via npm | default package pre-installed in the payload (v7) so no on-device npm runs; version string fixed (TESTED `P9_VERSION`) | seed: TESTED (`P9_PLUGIN PASS`, run 2, zero exit-159). **Extra user plugins: BLOCKED** - on-device Arborist SIGSYS-crashes Bun (run 1: 113 restarts) |
...
- Runtime internals live in app-private storage; project files live in the app's
  own directory on shared storage so they can be read with `adb`/MTP (see
  docs/ARCHITECTURE.md for exactly what that does and does not expose). No root,
  no Termux, no user install of Node/Bun.
