...
| **File write** (write/edit/patch tools) | native fs | same, inside the project directory (`Android/data/<pkg>/files/workspaces/<project>` since the Phase 10 continuation) | TESTED (P5 G-gates, P7 W2 boundary, P10 W4 + non-root shell read) |
| **Seeing the agent's files** | the filesystem itself | in-app file browser (OpenCode's `/file` API), `adb pull` for the project path, SAF "Publish to a folder" for a copy any file manager can open | TESTED (P6-U9 file browser gate; P10 visibility gates) |
...
| **Project / workspace** (cwd = project, multiple projects, worktrees) | directory chosen at launch | Projects screen; each project = its own directory under the app-specific external root = its own OpenCode instance (`?directory=`); SAF import/export/publish; isolation gate | TESTED (P7 W1-W3, re-run on the new root: P10_WS_*). Worktrees: not exposed |
