...
| **Plugins** (`@opencode-ai/plugin`, npm-installed on demand) | installed by upstream via npm | same code path; needed the bare version string (Phase 9 payload v6) | IMPLEMENTED (fix), **NOT TESTED** until `P9_PLUGIN` runs with registry egress; v5 payload was BLOCKED (`NpmInstallFailedError`) |
