...
log "=== early diagnostics (app uid) ==="
{
  ...
  echo "--- bin/git and bin/rg symlink targets (Android/Bionic executables) ---"
  rash "ls -l '$FILES/bin/git' '$FILES/bin/rg' 2>&1"
  echo "--- direct Git/ripgrep version checks from app uid ---"
  rash "cd '$WORKDIR' && '$FILES/bin/git' --version 2>&1; echo git_status_exec_rc=\$?"
  rash "cd '$WORKDIR' && '$FILES/bin/rg' --version 2>&1; echo rg_status_exec_rc=\$?"
  echo "--- Git repository status from app uid (same binaries the server resolves) ---"
  rash "cd '$WORKDIR' && '$FILES/bin/git' status --short 2>&1; echo git_status_rc=\$?"
  echo "--- seccomp/trap diagnostics (Android logcat, excluding expected SELinux audit noise) ---"
  adb logcat -c 2>/dev/null || true
  rash "cd '$WORKDIR' && '$FILES/bin/rg' --version >/dev/null 2>&1; '$FILES/bin/git' --version >/dev/null 2>&1; echo tool_diag_rc=\$?"
  sleep 1
...
# on the supervisor stderr). Also snapshot the workspace + opencode state dir.
{
  ...
  rash 'for d in "'"$FILES"'/xdg/data/opencode/log" "'"$FILES"'/xdg/state/opencode/log"; do
          for f in "$d"/*.log; do
            [ -e "$f" ] || continue
            echo "### $f"
...
rash 'for d in "'"$FILES"'/xdg/data/opencode/log" "'"$FILES"'/xdg/state/opencode/log"; do
        for f in "$d"/*.log; do
          [ -e "$f" ] || continue
          echo "### $f"; tail -150 "$f" 2>/dev/null
        done
      done' > "$EV/opencode-server.log" 2>/dev/null || true
