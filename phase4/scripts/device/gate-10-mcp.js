...
// Phase 4: MCP payload lives in app-private storage (provisioned via run-as).
const MCP_DIR = process.env.OPENCODE_MCP_DIR || "/data/local/tmp/gates/mcp"
const BUN_BIN = process.env.OPENCODE_BUN_BIN || "/data/local/tmp/gates/bin/bun"
try { mkdirSync(MCP_DIR, { recursive: true }) } catch {}
const out = await new Promise((resolve, reject) => {
  const child = spawn(BUN_BIN, [MCP_DIR + "/mcp-roundtrip.js"], { cwd: MCP_DIR })
  let so = "", se = ""
