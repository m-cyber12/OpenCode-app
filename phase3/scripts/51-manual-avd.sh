#!/usr/bin/env bash
# 51-manual-avd.sh — create the gates AVD by writing config files directly
...
AVD_DIR="$ANDROID_AVD_HOME/gates.avd"
mkdir -p "$AVD_DIR"

cat > "$AVD_DIR/config.ini" <<EOF
avd.ini.displayname=gates
hw.cpu.arch=x86_64
hw.cpu.ncore=2
hw.ramSize=2048
hw.gpu.enabled=no
...
cat > "$ANDROID_AVD_HOME/gates.ini" <<EOF
avd.ini.encoding=UTF-8
path=$AVD_DIR
path.rel=avd/gates.avd
target=$API
EOF
