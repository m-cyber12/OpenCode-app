...
class OpenCodeClientGatesTest {
    ...
    /**
     * Skip (not fail) when the runtime never came up — readiness is P5-01/02/04's
     * verdict, not this gate's. The println matters: an ignored test used to be
     * invisible, so run #13 reported `kotlin_gates_pass=0 fail=0` and looked like
     * broken gate logic instead of a server that had never started.
     */
    private fun requireServer(id: String) {
        if (serverUp()) return
        println("P5_$id SKIPPED :: app-owned OpenCode server not answering /global/health")
        Assume.assumeTrue("skipped: the app-owned OpenCode server is not answering /global/health", false)
    }
    ...
    fun k6_secretsAreKeystoreBackedAndNeverPlaintext() {
        ...
        try {
            ...
            // (b') Format contract, asserted because the platform, not us, supplies
            // the IV: OCS2 magic | version 2 | ivLen(2) | iv(12) | ct+tag. An earlier
            // revision wrote its own random IV and was rejected by AndroidKeyStore
            // ("Caller-provided IV not permitted"), which stopped the whole runtime.
            val bytes = blob.readBytes()
            assertEquals("blob magic", "OCS2", String(bytes, 0, 4, Charsets.ISO_8859_1))
            assertEquals("blob format version", 2, bytes[4].toInt())
            val ivLen = ((bytes[5].toInt() and 0xff) shl 8) or (bytes[6].toInt() and 0xff)
            assertEquals("AES-GCM IV length generated by the Keystore", 12, ivLen)
            assertEquals("ciphertext length must be plaintext+binding+tag",
                bytes.size - 7 - ivLen,
                marker.length + "ai.opencode.android/secret/provider-gatecanary".length + 1 + 16)
            ...
            // (e) slot binding: the same ciphertext under a different name must not open
            ...
            copy.delete()
            assertTrue("renamed blob must be rejected (name bound inside ciphertext)",
                swapped.isFailure)
            gate(
                "KEYSTORE",
                true,
