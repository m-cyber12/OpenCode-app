#!/usr/bin/env bash
# 52-boot-emulator.sh — boot the spike AVD headless and wait for boot completion.
set -uo pipefail
...
nohup "$EMU" -avd spike -no-window -no-audio -no-boot-anim -no-snapshot \
  -gpu swiftshader_indirect -memory 2048 -cores 2 -accel auto \
  > "$OUT/emulator.log" 2>&1 &
EMU_PID=$!
echo "emulator pid=$EMU_PID"

adb start-server 2>&1 | tee -a "$OUT/52-boot-emulator.log"
...
echo "--- boot state"
adb devices | tee -a "$OUT/52-boot-emulator.log"
if [ "$BOOTED" = 1 ]; then
