#!/usr/bin/env bash
# 30-static-checks.sh - (Phase 9 copy of the Phase 8 checker) everything Phase 9 can verify WITHOUT an emulator, run
...
echo "=== Phase 9 static checks $(date -u +%FT%TZ) ===" | tee -a "$LOG"

# ---- 1. shell + python syntax of every phase script --------------------------
find "$ROOT/spike" "$ROOT/phase3" "$ROOT/phase4" "$ROOT/phase5" "$ROOT/phase6" "$ROOT/phase7" "$ROOT/phase8" "$ROOT/phase9" \
...
done < <(find "$ROOT/phase5" "$ROOT/phase6" "$ROOT/phase7" "$ROOT/phase8" "$ROOT/phase9" -name '*.py' -type f 2>/dev/null | sort)
...
if command -v node >/dev/null 2>&1; then
  JSSYN=0
  for f in "$ROOT"/phase8/scripts/device/*.js "$DIR"/scripts/device/*.js; do
    [ -f "$f" ] || continue
    node --check "$f" >> "$LOG" 2>&1 || { echo "FAIL node --check $f" | tee -a "$LOG"; JSSYN=1; }
  done
  if [ "$JSSYN" = 0 ]; then echo "OK   node --check on every phase8/phase9 driver" | tee -a "$LOG"; else RC=1; fi
else
...
# Only the phase9 template is checked: the phase7/phase6/... templates pin their
...
# ---- 6. Phase 9 release invariants ------------------------------------------
run "versions.lock == RuntimeVersion.kt == build.gradle versionName (shipped == pinned)" \
  python3 "$ROOT/phase9/scripts/check-lock.py" "$ROOT"
run "release docs present (README, ARCHITECTURE, RUNTIME, SECURITY, TESTING, capability matrix)" \
  bash -c 'for f in README.md docs/ARCHITECTURE.md docs/RUNTIME.md docs/SECURITY.md docs/TESTING.md docs/CAPABILITY-MATRIX.md; do [ -s "$ROOT/$f" ] || { echo "missing $f"; exit 1; }; done'
run "no OPENCODE_VERSION define with an -android suffix (plugin-install bug, Phase 9)" \
  bash -c '! grep -n "OPENCODE_VERSION.*-android" "$ROOT/phase4/scripts/10-build-payload.sh"'
